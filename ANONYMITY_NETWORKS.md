# Anonymity Networks with Dtor

Currently only Tor and I2P have been integrated into Dtor. The usage of
...
is made or until `dtord` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `dtor-wallet-cli` and
`dtor-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `dtord` the ip/port of a socks proxy provided by a
...
which tells `dtord` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `dtord` the inbound address, network
...
which tells `dtord` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `dtord`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `dtord` localhost port 30000.
...
`dtord` RPC port - which is independent from the configuration for incoming
...
```
HiddenServiceDir /var/lib/tor/data/dtor
...
This will store key information in `/var/lib/tor/data/dtor` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/dtor/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Dtor network should have
similar timestamps.
...
### Intermittent Dtor Syncing

If a user only runs `dtord` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `dtord` as often as possible to conceal when transactions are being sent.
...
`dtord` currently selects two outgoing connections every 5 minutes for
...
`dtord` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `dtord` should likely disconnect
