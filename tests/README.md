...
Core tests take longer than any other Dtor tests, due to the high amount of computational work involved in validating core components.
...
To run only Dtor's core tests (after building):
...
To run only Dtor's crypto tests (after building):
...
```
dtord --regtest --offline --fixed-difficulty 1
```
...
To run only Dtor's hash tests (after building):
...
To run only Dtor's performance tests (after building):
...
To run only Dtor's unit tests (after building):
